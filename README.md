...
время insert/update для 100тыс ![](imgs/bench3.png)

итого 1 шард выдерживает около 20тыс rps

Для того чтобы база выдеражала такое кол-во rps воспользуемя Шардирование, разобьём базу на 20 шардов.
Каждый шард будет относится к своей геозоне.
...
Для отказоустойчивости воспользуемся репликацией каждый шард будет представлять из себя 1 мастер и 2 реплики,
...
Нам [достаточно](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/) 1 сервера, но для надежности возьмем 2 c характеристиками:
...
### Бэкенд
За секунду 32 ядерный сервер в целом может держать 40 000 RPS.Возьмём 1 для основного бекенда и 4 для сервиса геолокация Для отказоустойчивости возьмем в 2 раза больше. Характеристики возьмем следующие:
...
Нам понадобится 10 серверов и 20 на реплики для геолакация и 4 сервера для остальных данных

Сервер для хранения геолакаций

| CPU(cores) | RAM(GB) | SSD(TB) |
|------------|---------|---------|
|   16      | 32  | 5    |

Сервер для хранения остальных данных
